...
def applyOperations(nums: list[int]) -> list[int]:
    ...
    for i in range(n - 1):                                          # Iterate through the list range
        if nums[i] == nums[i + 1]:                                  # Check if neigh-ele are equal
            nums[i] *= 2                                            # Update the element at curr-idx
            nums[i + 1] = 0                                         # Update the neigh-ele to zero

    for num in nums:                                                # Iterate through the updated list
        if num: res.append(num)                                     # Append the num to result, if non-zero

    # Merge a list of zeroes to balance the elements to the original amount and return the result
