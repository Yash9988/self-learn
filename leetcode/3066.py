...
It can be shown that 4 is the minimum number of operations needed so that all elements of the array are greater than or
equal to 20.
...
def minOperations(nums: list[int], k: int) -> int:
    ...
    while nums[0] < k:                                              # While the smallest element is less than `k`
        x, y = heappop(nums), heappop(nums)                         # Pop out the two smallest elements from the heap
        heappush(nums, min(x, y) * 2 + max(x, y))                   # Compute & push the new element back into the heap
