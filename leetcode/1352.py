...
class ProductOfNumbers:
    def __init__(self):
        self.prod = [1]                                             # List to track prefix-product

    def add(self, num: int) -> None:
        if not num:                                                 # Check if the curr-num is zero
            self.prod = [1]                                         # Reset the list
        else:
            self.prod.append(self.prod[-1] * num)                   # Compute and append the new product, otherwise

    def getProduct(self, k: int) -> int:
        if k >= len(self.prod):                                     # Check if `k` is greater than the list size
            return 0                                                # Return zero

        return self.prod[-1] // self.prod[-k - 1]                   # Compute and return the result
