...
array classes, where classes[i] = [pass_i, total_i]. You know beforehand that in the ith class, there are total_i total
students, but only pass_i number of students will pass the exam.
...
def maxAverageRatio(classes: list[list[int]], extraStudents: int) -> float:
    # Helper method to computer profit we can get if we add one extraStudents to a particular class
    def profit(a, b):
        return ((a + 1) / (b + 1)) - a / b

    heap = [(-profit(a, b), a, b) for a, b in classes]              # List to store custom-tuples: (profit, pass, total)
    heapify(heap)                                                   # Convert the list into a heap

    for _ in range(extraStudents):                                  # Iterate through the range of extraStudents
        _, a, b = heappop(heap)                                     # Pop out the class tuple with max-profit

        # Assign student to this class. Compute and push the new-tuple back into the heap
        heappush(heap, (-profit(a + 1, b + 1), a + 1, b + 1))

    return sum(a / b for _, a, b in heap) / len(classes)            # Return the result
