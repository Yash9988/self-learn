...
Concepts: Cantor's Diagonalization
"""
...
def findDifferentBinaryString(nums: list[str]) -> str:
    ...
    for i in range(n):                                              # Iterate through the range of list size
        # Update the res-str based upon the i-th bit of the i-th string
        res += '1' if nums[i][i] == '0' else '0'

    return ''.join(res)                                             # Return the result
