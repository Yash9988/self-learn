...
def getSneakyNumbers(nums: list[int]) -> list[int]:
    res, seen = [], set()                       # Initialise a set for tracking seen and list to store result

    for n in nums:                              # Iterate through all numbers
        if n in seen:                           # Check if number was seen already
            res.append(n)                       # Append the number in result array
        else:
            seen.add(n)                         # Add the number to the seen-set, otherwise

    return res                                  # Return the result


# TODO: Possible "Medium" way of solving the problem using bit-manipulation
