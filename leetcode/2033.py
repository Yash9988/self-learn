...
def minOperations(grid: list[list[int]], x: int) -> int:
    arr = [num for row in grid for num in row]                      # Flatten the grid into a single list
    arr.sort()                                                      # Sort the list

    median = arr[len(arr) // 2]                                     # Compute the median
    ...
    return sum(abs(num - median) // x for num in arr)               # Calculate the minimum number of operations
