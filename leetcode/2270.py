...
def waysToSplitArray(nums: list[int]) -> int:
    total = sum(nums)                                               # Compute the sum of the list
    acc = res = 0                                                   # Counters to track prefix sum and result

    for n in nums[:-1]:                                             # Iterate through the list
        acc += n                                                    # Increment the counter by ele-val
        total -= n                                                  # Decrement the total by the ele-val
        res += acc >= total                                         # Increment counter if the condition is satisfied
