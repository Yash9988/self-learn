...
Let's define a boolean function isPrefixAndSuffix that takes two strings, str1 and str2:
    ...
    - isPrefixAndSuffix(str1, str2) returns true if str1 is both a _prefix_ and a _suffix_ of str2, and false otherwise.
...
def countPrefixSuffixPairs(words: list[str]) -> int:
    # Method to verify if a given word is both a prefix and a suffix
    ...
    for i in range(n):                                              # Iterate through the list range
        for j in range(i + 1, n):                                   # Iterate through the remaining list range
            res += isPrefixAndSuffix(words[i], words[j])            # Increment the counter, as per the method output
