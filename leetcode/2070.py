...
def maximumBeauty(items: list[list[int]], queries: list[int]) -> list[int]:
    items = sorted(items + [[0, 0]])                                    # Modify and sort the list

    for i in range(len(items) - 1):                                     # Iterate through all the items
        items[i + 1][1] = max(items[i][1], items[i + 1][1])             # Update each item with the max-beauty so far

    return [items[bisect(items, [q + 1]) - 1][1] for q in queries]      # Use bisect to find max-val for each query
