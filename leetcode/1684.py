...
def countConsistentStrings(allowed: str, words: list[str]) -> int:
    allowed = set(allowed)                                          # Create a set of allowed characters
    consistent = 0                                                  # Initialise counter

    for word in words:                                              # Iterate through all words
        consistent += allowed.union(set(word)) == allowed           # Increment if all characters belong in allowed-set

    return consistent                                               # Return the result
