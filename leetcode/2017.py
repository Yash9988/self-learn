...
def gridGame(grid: list[list[int]]) -> int:
    firstSum = sum(grid[0])                                         # Compute the sum of the first row
    secondSum = 0                                                   # Counter to track the selective sum of second row

    res = float('inf')                                              # Counter to track the result
    for i in range(len(grid[0])):                                   # Iterate through the cols
        firstSum -= grid[0][i]                                      # Decrement the total by idx-ele-val from first row

        res = min(res, max(firstSum, secondSum))                    # Update the result with the min of max-vals
        secondSum += grid[1][i]                                     # Increment counter by idx-ele-val from second row
